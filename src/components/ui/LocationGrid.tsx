...
export function LocationGrid() {
  return (
    <div className="inline-flex items-center gap-2.5 border border-line rounded-[3px] px-6 py-5">
      ...
      <div>
        <span className="block text-[13px] text-ink tracking-[-0.01em]">{locations[0].name}</span>
        <span className="block text-[11px] text-ink-muted mt-0.5">{locations[0].region}</span>
      </div>
    </div>
  );
}
